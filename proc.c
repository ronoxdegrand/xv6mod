...
// [EXPERIMENTAL] Waitx for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){ 
      ...
      for(q = ptable.proc;q < &ptable.proc[NPROC];q++)
        if((q->state == RUNNABLE) && (high_priority->priority > q->priority))
          high_priority = q;

      if(high_priority->priority > q->priority)
        high_priority = q;
...
scheduler(void)
{
  cprintf("Scheduler: First Come, First Serve\n");
...
getprocs(struct ProcessInfo* processInfoTable){
  ...
  for (i = 0, p = ptable.proc; p < &ptable.proc[NPROC] && i < NPROC; i++,p++)
  {
    ...
    processInfoTable[i].rtime = p->rtime;
    processInfoTable[i].ctime = p->ctime;
    processInfoTable[i].etime = p->etime;
